...
def _init_code_site(tmp_path):
    ...
    (lectures_dir / "demo.py").write_text(
        "value = 3\n"
        "print(value)\n"
    )
    (lectures_dir / "rectangle.py").write_text(
        "name = 'lowercase'\n"
    )
...
class TestCodeProseLinksRewritten:
    ...
    def test_copied_java_has_full_urls(self, built_prod_site):
        ...
        java_file = (
            built_prod_site / "dist-prod" / "v1" / "lectures" / "01" / "Linked.java"
        )
    ...
    def test_prose_source_has_full_urls(self, built_prod_site):
        ...
        html_file = (
            built_prod_site / "dist-prod" / "v1" / "lectures" / "01" / "Linked.java.html"
        )
        html = html_file.read_text()

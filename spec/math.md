...
Each rendered math expression includes an `aria-label` with a screen-reader-
friendly description generated by `katex/contrib/render-a11y-string`. For
example, `$\frac{1}{2}$` gets the label
`"start fraction, 1, divided by, 2, end fraction"`. This supplements the
MathML that KaTeX already embeds in its output.
